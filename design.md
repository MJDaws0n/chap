...
## UI Design System (2025 Overhaul)

This section is the source of truth for the web UI look & feel. It is intentionally focused on:

- A consistent, modern “glass” UI with blur + translucency.
- Strong readability in both light and dark.
- Mobile-first layout choices and safe overflow handling.
- Zero Tailwind and zero SweetAlert.

### Theme

- Theme modes: `auto` (system), `light`, `dark`.
- Persisted in `localStorage` under key `chap.theme`.
- Applied by setting `data-theme` on the `<html>` element.

Implementation:

- Theme JS: [server/public/js/theme.js](server/public/js/theme.js)
- Theme UI: the segmented control (`.theme-switch`) in layouts.

### Tokens

All colors, radii, blur and shadow tokens are CSS variables defined in:

- [server/public/css/tokens.css](server/public/css/tokens.css)

Rules:

- Use tokens (`--bg`, `--surface`, `--border`, `--text`, `--accent`, etc.) instead of hard-coding colors in new CSS.
- Prefer translucent surfaces (`--surface`, `--bg-elev`) over solid fills.
...
- `.app-shell` provides sidebar + main.
- Sidebar becomes an overlay on smaller screens; the overlay uses `body[data-sidebar-open="true"]`.

Files:

- [server/public/css/layout.css](server/public/css/layout.css)
...
- [server/public/css/components.css](server/public/css/components.css)

Conventions:

- Buttons: `.btn`, `.btn--primary`, `.btn--danger`, `.btn--ghost` (avoid per-page button styles).
- Surfaces: `.card` + `.card__header` + `.card__body`.
- Form fields: `.field` + `.label` + `.input` / `.select` / `.textarea`.
- Alerts: `.alert`, `.alert--success`, `.alert--error`.

### Popups (SweetAlert replacement)

SweetAlert2 is removed. We keep the existing `chapSwal({...}).then(...)` calling pattern but it now uses our own modal implementation:

- [server/public/js/chapSwal.js](server/public/js/chapSwal.js)

Behavior compatibility goals:

- Resolves a Promise with `{ isConfirmed, isDismissed, value }`.
- Supports `title`, `text`, `html`, `showCancelButton`, `confirmButtonText`, `cancelButtonText`, `preConfirm`, `didOpen`.

### Mobile + overflow rules

- All flex rows that contain potentially long text must allow shrink (`min-width: 0`) and use truncation where needed.
- Inputs/textareas must never overflow the viewport; use `width: 100%` and `min-width: 0`.
- Live logs must wrap safely: use `white-space: pre-wrap` + `overflow-wrap: anywhere` + `word-break: break-word`.

### Migration note (temporary)

During the migration, a small compatibility layer exists to keep pages readable while markup is converted:

- [server/public/css/utilities.css](server/public/css/utilities.css)

New UI should prefer semantic classes rather than adding more “utility” mappings.
